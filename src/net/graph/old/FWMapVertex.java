...
public class FWMapVertex extends Vertex<LongWritable, LongIntMapWritable, NullWritable, NullWritable> 
{
	private static Logger LOG = Logger.getLogger(FWMapVertex.class);
	...
	{		
		...
		setValue(i_);		
		if (i==s) aggregate(FWMapAggregator.ID, i_);
		if (V==s) voteToHalt();
	}
	...
	private void compute() throws IOException
	{
		...
		if (d_ik!=null)	
		{
			FWMapWorkerContext ctx = (FWMapWorkerContext) getWorkerContext();
	...
	private ArrayList<Compute> initPool(FWMapWorkerContext ctx)
	{
